...
const SESSIONS = [
  {
    ...
    poses: [
      { name: 'Mountain Pose', sanskrit: 'Tadasana', duration: 30, benefit: 'Improves posture and balance', instructions: 'Stand tall, feet together, arms at sides, breathe deeply', icon: '🏔️', color: '#fce0ec' },
      { name: 'Sun Salutation', sanskrit: 'Surya Namaskar', duration: 45, benefit: 'Energizes entire body', instructions: 'Flow through forward fold, plank, cobra, and back up', icon: '☀️', color: '#fff3cd' },
      { name: 'Warrior I', sanskrit: 'Virabhadrasana I', duration: 30, benefit: 'Builds strength and focus', instructions: 'Step one foot back, bend front knee, raise arms overhead', icon: '⚡', color: '#e8dff8' },
      { name: 'Tree Pose', sanskrit: 'Vrikshasana', duration: 30, benefit: 'Improves balance and concentration', instructions: 'Stand on one leg, place other foot on inner thigh, hands in prayer', icon: '🌳', color: '#d4f0e4' },
      { name: "Child's Pose", sanskrit: 'Balasana', duration: 45, benefit: 'Calms the mind and relieves stress', instructions: 'Kneel, sit back on heels, extend arms forward, rest forehead down', icon: '🌸', color: '#fce0ec' },
    ]
  },
  {
    id: 'stress',
    ...
    poses: [
      { name: "Child's Pose", sanskrit: 'Balasana', duration: 60, benefit: 'Deeply relaxes the nervous system', instructions: 'Kneel, sit back on heels, extend arms forward, breathe slowly', icon: '🌸', color: '#fce0ec' },
      { name: 'Cat-Cow Stretch', sanskrit: 'Marjaryasana', duration: 45, benefit: 'Releases spine tension', instructions: 'On hands and knees, alternate arching and rounding your back', icon: '🐱', color: '#d4f0e4' },
      { name: 'Seated Forward Fold', sanskrit: 'Paschimottanasana', duration: 45, benefit: 'Calms anxiety and stress', instructions: 'Sit with legs extended, reach forward toward your feet, breathe', icon: '🌊', color: '#c8e8f8' },
      { name: 'Legs Up The Wall', sanskrit: 'Viparita Karani', duration: 60, benefit: 'Reduces stress and fatigue instantly', instructions: 'Lie on back, extend legs up against wall, relax completely', icon: '🕊️', color: '#e8dff8' },
      { name: 'Corpse Pose', sanskrit: 'Savasana', duration: 90, benefit: 'Complete relaxation of body and mind', instructions: 'Lie flat on back, arms at sides, eyes closed, breathe naturally', icon: '🌙', color: '#e8dff8' },
    ]
  },
  {
    id: 'focus',
    ...
    poses: [
      { name: 'Easy Pose', sanskrit: 'Sukhasana', duration: 60, benefit: 'Centers the mind for focus', instructions: 'Sit cross-legged, spine tall, hands on knees, eyes closed', icon: '🧘', color: '#e8dff8' },
      { name: 'Eagle Pose', sanskrit: 'Garudasana', duration: 30, benefit: 'Improves concentration', instructions: 'Stand, cross one leg over other, wrap arms, focus on one point', icon: '🦅', color: '#c8e8f8' },
      { name: 'Warrior III', sanskrit: 'Virabhadrasana III', duration: 30, benefit: 'Builds mental strength', instructions: 'Balance on one leg, extend body parallel to floor, arms forward', icon: '⚡', color: '#e8dff8' },
      { name: 'Seated Meditation', sanskrit: 'Dhyana', duration: 120, benefit: 'Clears mental clutter', instructions: 'Sit comfortably, close eyes, focus only on your breath', icon: '✨', color: '#fce0ec' },
    ]
  },
  {
    id: 'sleep',
    ...
    poses: [
      { name: 'Butterfly Pose', sanskrit: 'Baddha Konasana', duration: 60, benefit: 'Releases hip tension from sitting', instructions: 'Sit, bring soles of feet together, gently flutter knees up and down', icon: '🦋', color: '#e8dff8' },
      { name: 'Supine Twist', sanskrit: 'Supta Matsyendrasana', duration: 45, benefit: 'Releases spine and promotes sleep', instructions: 'Lie on back, bring knee to chest, cross it over body, arms spread', icon: '🌀', color: '#c8e8f8' },
      { name: 'Happy Baby', sanskrit: 'Ananda Balasana', duration: 45, benefit: 'Calms the nervous system', instructions: 'Lie on back, grab outer feet, gently rock side to side', icon: '🌷', color: '#fce0ec' },
      { name: 'Legs Up The Wall', sanskrit: 'Viparita Karani', duration: 90, benefit: 'Reduces anxiety before sleep', instructions: 'Lie on back, legs up against wall, breathe slowly for full duration', icon: '🕊️', color: '#e8dff8' },
      { name: 'Corpse Pose', sanskrit: 'Savasana', duration: 120, benefit: 'Prepares body for sleep', instructions: 'Lie completely still, scan body from toes to head, release all tension', icon: '🌙', color: '#e8dff8' },
    ]
  },
]

function PoseVisual({ pose, progress }) {
  return (
    <div style={{ position: 'relative', width: 200, height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <motion.div
        ...
        style={{
          ...
          border: '2px solid rgba(167,139,202,0.4)',
        }}
      />
      ...
      <svg width="200" height="200" style={{ position: 'absolute', transform: 'rotate(-90deg)' }}>
        <circle cx="100" cy="100" r="90" fill="none" stroke="rgba(167,139,202,0.1)" strokeWidth="4" />
        <motion.circle
          cx="100" cy="100" r="90"
          fill="none"
          stroke="url(#poseGrad)"
        ...
        />
        <defs>
          <linearGradient id="poseGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <motion.div
        ...
        style={{
          ...
          background: `linear-gradient(135deg, ${pose.color}, rgba(167,139,202,0.2))`,
          display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 8px 32px rgba(167,139,202,0.25)',
          zIndex: 1,
        }}>
        <motion.span
      ...
      {[0, 1, 2, 3].map(i => (
        <motion.div key={i}
          ...
          style={{
            ...
            borderRadius: '50%', background: '#a78bca',
          }}
        />
      ))}
    </div>
          ...
          <motion.div key="pose" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div key={currentPose}
              ...
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
                <PoseVisual pose={pose} progress={progress} />
              </div>
